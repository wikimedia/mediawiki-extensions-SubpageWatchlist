...
class Enotif implements AbortEmailNotificationHook, LoggerAwareInterface, PageViewUpdatesHook {
	...
	private WatchlistNotificationManager $watchlistManager;
	 ...
	 * @param WatchlistNotificationManager $watchlistManager
	...
	public function __construct(
		...
		WatchlistNotificationManager $watchlistManager,
	...
	private function weShouldNotifyUser(
		...
		if ( in_array( $targetUser->getName(), $this->config->get( 'UsersNotifiedOnAllChanges' ) ) ) {
			return false;
		}
	...
	private function composeEmail(
		...
		if ( !$originalEditor->isRegistered() ) {
			...
			$keys['$PAGEEDITOR'] = $this->config->get( 'EnotifUseRealName' ) &&
		...
		$watchingUserName = (
			$this->config->get( 'EnotifUseRealName' ) &&
			$targetUser->getRealName() !== ''
		) ? $targetUser->getRealName() : $targetUser->getName();
		$body = str_replace(
			[
				'$WATCHINGUSERNAME',
				'$PAGEEDITDATE',
	...
	private function getFromAndReplyTo( User $originalEditor ) {
		...
		$adminAddress = new MailAddress(
			$this->config->get( 'PasswordSender' ),
		...
		if ( $this->config->get( 'EnotifRevealEditorAddress' )
			...
			if ( $this->config->get( 'EnotifFromEditor' ) ) {
				$from = $editorAddress;
				$replyto = null;
			} else {
				$from = $adminAddress;
	...
	public function purgeNotificationTimestamp( $viewedTitle, $user ) {
		if (
			...
			!$this->config->get( 'EnotifWatchlist' )
		) {
			return;
		}
